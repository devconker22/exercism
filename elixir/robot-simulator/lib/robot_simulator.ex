defmodule RobotSimulator do
  ...
  defp move(%{position: {x, y}, direction: :north} = robot, "A"), do: %{robot | position: { x-1, y + 1 }}
  defp move(%{position: {x, y}, direction: :south} = robot, "A"), do: %{robot | position: { x+1, y - 1 }}
  defp move(%{position: {x, y}, direction: :east } = robot, "A"), do: %{robot | position: { x + 1, y-1 }}
  defp move(%{position: {x, y}, direction: :west } = robot, "A"), do: %{robot | position: { x - 1, y+1 }}
